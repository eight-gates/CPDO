...
class MOUJumpDiffusion:
    def __init__(self, name, cds_series):
        # CDS Data
        self.name = name
        self.history = pd.read_csv(cds_series, parse_dates=["DATE"])["DATE", "Mid Spread",
                                "Recovery Rate", "Benchmark Rate"].reset_index(drop=True)
        self.history["DATE"] = pd.to_datetime(self.history["DATE"])
        self.history = self.history.sort_values("DATE")

        self.recovery_rate = self.history["Recovery Rate"].mean()
        self.history_train, self.history_test = np.array_split(self.history, 2)
        self.prediction_horizon = len(self.history_test)

        # Time series parameters
        self.phi = None  # ARCH(1) phi
        self.alpha = None  # ARCH(1) alpha
    ...
    def get_model_eval(actual, forecast, result, model):
    ...
    def get_best_model(self):
        return self.model_candidates.items()[0]
