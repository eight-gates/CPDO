...
class MultiCDSSimulator:
    def __init__(self, name_to_series_map, nu=2):
        # build a dict of name -> MOUJumpDiffusion
        self.cds_basket = {
            name: MOUJumpDiffusion(name, series)
            for name, series in name_to_series_map.items()
        }
         # names simply come from the keys of that dict
        ...
        if torch.has_mps:
            self.L_t = torch.tensor(self.L, device=torch.device('mps'), dtype=torch.float32)
        else:
            self.L_t = None
